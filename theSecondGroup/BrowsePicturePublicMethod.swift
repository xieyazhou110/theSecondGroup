...
class BrowsePicturePublicMethod: NSObject {
    ...
    //传进来大小图url的数组
    class func showBigPictureWithUrl(smallImageUrl:[String],bigImageUrl:[String],rect:CGRect,controller:UIViewController,completion: ((Int) -> Void)?){
        ...
        if smallImageUrl.count == 0 && bigImageUrl.count > 0{
            BrowsePicturePublicMethod.showBigPictureOnlyBigWithUrl(bigImageUrl: bigImageUrl,rect: rect, controller: controller)
            return
        }
...
//            model.smallImage = image
        ...
        tempBrowsePicturePublicMethod.showBigPicture(imageModelArray:tempBrowsePicturePublicMethod.modelArray,rect: rect,controller: controller)
        if let completion = completion {
            completion(5)
        }
    }
    //只传进来大图的url
    class func showBigPictureOnlyBigWithUrl(bigImageUrl:[String],rect:CGRect,controller:UIViewController){
        let tempBrowsePicturePublicMethod = BrowsePicturePublicMethod()
    ...
    //直接过来大小UIImage
    class func showBigPictureWithImage(smallImage:[UIImage],bigImage:[UIImage],rect:CGRect,controller:UIViewController,completion: ((Int) -> Void)?) {
        let tempBrowsePicturePublicMethod = BrowsePicturePublicMethod()
        //当两个数组都有值，但是数量不一致直接retrun,防止出现数组越界
        ...
        if smallImage.count == 0 && bigImage.count > 0{
            BrowsePicturePublicMethod.showBigImagePictureOnlyBigImage(imageArray: bigImage,rect:rect,controller: controller)
            return
        }
    ...
    //只传过来只有大图的数组
    class func showBigImagePictureOnlyBigImage(imageArray:[UIImage],rect:CGRect,controller:UIViewController) {
        let tempBrowsePicturePublicMethod = BrowsePicturePublicMethod()
    ...
    //
    fileprivate func showBigPicture(imageModelArray:[ImageModel],rect:CGRect,controller:UIViewController){
